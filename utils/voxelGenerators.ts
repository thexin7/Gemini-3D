...
/**
 * 辅助函数：设置方块
 * 原理：使用 Map 结构，以坐标 "x,y,z" 字符串作为 Key。
 * 这保证了空间中同一个整数坐标点只能存在一个方块，避免重复堆叠。
 */
...
/**
 * 辅助函数：生成球体/椭球体
 * 原理：遍历包围盒内的所有整数点，利用球体方程 (x-cx)^2 + (y-cy)^2 + (z-cz)^2 <= r^2 判断点是否在球内。
 * 
 * @param sy - Y 轴缩放因子。sy > 1 会压扁球体，sy < 1 会拉长球体 (变成椭球)。
 */
function generateSphere(map: Map<string, VoxelData>, cx: number, cy: number, cz: number, r: number, col: number, sy = 1) {
    const r2 = r * r;
    // 优化：只遍历包围盒 (Bounding Box) 内的点，而不是整个空间
...
/**
 * 程序化生成器集合 (Procedural Generators)
 * 这里的代码通过硬编码的数学公式生成初始的 3D 模型。
 * 相比 AI 生成，这些模型的结构是确定的、计算极快的，适合作为初始演示。
 */
export const Generators = {
    Eagle: (): VoxelData[] => {
        const map = new Map<string, VoxelData>();
        // 生成树枝：使用正弦波曲线 (Sine Wave) 模拟自然的弯曲感
        for (let x = -8; x < 8; x++) {
